...
$realName = $_SESSION['name'];
    ...
    $name = $_SESSION['name'];
    $query = "SELECT * FROM `ncst-positions`;";
...
if (isset($_POST['add'])) 
{
    ...
    $add="INSERT INTO `ncst-positions`(`position`) VALUES ('$addPos')";
    $addQuery=mysqli_query($connection,$add);
       
    if($addQuery){
      $_SESSION['status'] = "Position Successfully Added!";
      $_SESSION['status_code'] = "success";
      header('Location: Admin-position.php');
      exit(0);

    }else{
      $_SESSION['status'] = "Position Not Added";
...
if (isset($_POST['update'])) 
{
     ...
     $newId=$_POST['position_id'];
     $posName=$_POST['position_name'];
     
    $update="UPDATE `ncst-positions` SET `position`='$posName' where id=$newId;";
    $updateQuery=mysqli_query($connection,$update);
       
    if($updateQuery){
      $_SESSION['status'] = "Position Updated";
      $_SESSION['status_code'] = "success";
      header('Location: Admin-position.php');
      exit(0);

    }else{
      $_SESSION['status'] = "Position Not Updated";
      $_SESSION['status_code'] = "error";
      header('Location: Admin-position.php');
    }   
}
...
if (isset($_POST['delete'])) 
{
    ...
    $delete="DELETE FROM `ncst-positions` WHERE id=$deleteId;";
    $deleteQuery=mysqli_query($connection,$delete);
       
    if($deleteQuery){
      $_SESSION['status'] = "Position has been Deleted!";
      $_SESSION['status_code'] = "success";
      header('Location: Admin-position.php');
      exit(0);

    }else{
      $_SESSION['status'] = "Position Not Deleted";
      $_SESSION['status_code'] = "error";
      header('Location: Admin-position.php');
    }   
}
  ...
  <div id="collapseTwo" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
    <div class="bg-white py-2 collapse-inner rounded">
      <h6 class="collapse-header">Manage:</h6>
        ...
        <a class="collapse-item active" href="Admin-position.php"><i class="fas fa-briefcase"></i> Positions</a>
    </div>
  </div>
</li>
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="h4 m-0 font-weight-bold text-primary">Position List
<!-- ADD Button trigger modal -->          
<span class="float:right"><button type="button" class="btn btn-primary mb-2 float-right" data-toggle="modal" data-target="#position">
<i class="fa fa-plus"></i> Add Position
</button></span>
<!-- START ADD BTN MODAL ====================================================================================== -->
<div class="modal" id="position" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="Admin-position.php" method="post">
        <div class="container-fluid">
            <div class="form-group">
            <h6 style="float: left;">Position Name</h6>
          <input class="form-control" type="text" name="addPos" value="" required/>
            </div>
        </div>
      ...
      <div class="modal-footer">
        ...
        <input type="submit" class="btn btn-primary" id="add" name="add" value="Add Position">
        </form>
      </div>
    </div>
  </div>
...
td {
  ...
  ?>
    ...
    <tr>
      ...
      <td>
      <center>
          <button type="button" class="btn btn-success btn-md" data-toggle="modal" data-target="#edit<?php echo $row['id']; ?>">
          Edit</button>
    ...
    <div class="modal-content">
      ...
      <form action="Admin-position.php" method="post">
        ...
        <input type="hidden" name="position_id" value="<?php echo $row['id']; ?>">
            <div class="form-group">
            <h6 style="float: left;">Position Name</h6>
          <input class="form-control" type="text" name="position_name" value="<?php echo $row['position'];?>"/>
            </div>
        </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <input type="submit" class="btn btn-success" id="update" name="update" value="Save changes">
        </form>
      </div>
    </div>
  </div>
    ...
    <div class="modal-content">
      ...
      <form action="Admin-position.php" method="post">
        ...
        <input type="hidden" name="delete_id" value="<?php echo $row['id']; ?>">
            <div class="form-group">
            <h6 style="float: left;">Are you sure you want to delete this Position?</h6>
            </div>
        </div>
