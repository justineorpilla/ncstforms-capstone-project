<?php // PENDING-8.PHP FOR SUBSTITUTION TO NOTE (FOR: COLLEGE DEAN)
...
}elseif($_SESSION['position']!='College Dean'){
  echo "<script>window.open('index.php', '_self')</script>";
}else{
...
if (isset($_POST['approve'])) 
{
    ...
    $noter_signature = $realSignature;
    
    $approve="UPDATE `substitutions` SET `status` = 'recommended', `noted_at` = CURRENT_TIMESTAMP, `noted_by` = '$realName', `noter_signature` = '$noter_signature' WHERE `id` = '$approve_id';";
    $approveQuery=mysqli_query($connection,$approve);
       
    if($approveQuery){
      $_SESSION['status'] = "Substitution has been Noted!";
      $_SESSION['status_code'] = "success";
      header('Location: pending-8.php');
      exit(0);
...
if (isset($_POST['reject'])) 
{
    ...
    $reject="UPDATE `substitutions` SET `status` = 'rejected', `approved_at` = CURRENT_TIMESTAMP, `approved_by` = '$realName', `remarks` = '$remarks', `approver_signature` = '$reject_signature' WHERE `id` = '$delete_id';";
...
<!-- =======================================================================================================================-->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="pending-8.php" method="post">
        <div class="container-fluid">
        <input type="hidden" name="accept_id" value="<?php echo $row['id']; ?>">
            <div class="form-group">
          <h5>Are you sure you want to recommend this Substitution Request?</h5>
          <small><p style="color:green;">(This form will be sent to the Dept. Coordinator for final approval)</p></small>
            </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form action="pending-8.php" method="post">
        ...
        <input type="hidden" name="delete_id" value="<?php echo $row['id']; ?>">
            ...
            <input class="form-control" type="text" placeholder="Remarks" name="remarks" required>
            </div>
        </div>
