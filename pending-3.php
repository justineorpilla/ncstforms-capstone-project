<?php // PENDING-3.PHP FOR PENDING LEAVES TO FIRST APPROVE (USER LEVEL: CAMPUS MANAGER)
...
}elseif($_SESSION['position']!='Campus Manager'){
  echo "<script>window.open('index.php', '_self')</script>";
}else{
...
if (isset($_POST['approve'])) 
{
    ...
    $approve="UPDATE `leaves` SET `status` = 'campus', `approve_manager_at` = CURRENT_TIMESTAMP, `approve_manager` = '$realName', `approver_signature` = '$realSignature' WHERE `id` = '$approve_id';";
    $approveQuery=mysqli_query($connection,$approve);
       
    if($approveQuery){
      $_SESSION['status'] = "Leave has been Approved!";
      $_SESSION['status_code'] = "success";
      header('Location: pending-3.php');
      exit(0);
...
if (isset($_POST['reject'])) 
{
    ...
    $reject="UPDATE `leaves` SET `status` = 'rejected', `approved_at` = CURRENT_TIMESTAMP, `approved_by` = '$realName', `remarks` = '$remarks', `reject_signature` = '$realSignature' WHERE `id` = '$delete_id';";
...
<!--=====================================================================================================================-->
           ...
           <div class="tab-content mt-3">
            <div class="tab-pane active" id="leave" role="tabpanel">
              <!-- Start -->
<?php
$query = "SELECT * FROM `leaves` WHERE `status` IN ('recommended','endorsed');";
...
<?php if ($row['type']=='Sick Leave') { ?>
<td>
         Recommended by: <b><?php echo $row['recommended_by']; ?></b>, <small>Supervisor</small> <br>
         Endorsed by:  <b><?php echo $row['endorsed_by']; ?></b>, <small>School Nurse</small> <br>
         First Approval:  <b>Pending</b> <br>
         Final Approval: N/A
</td>
<?php }else{ ?>
<td>
...
<!-- =======================================================================================================================-->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="pending-3.php" method="post">
        <div class="container-fluid">
        <input type="hidden" name="accept_id" value="<?php echo $row['id']; ?>">
            <div class="form-group">
          <h5>Are you sure want to approve this Leave?</h5>
          <small><p style="color:green;">(This Leave will be sent to the
           to the Administrator for final approval)</p></small>
            </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form action="pending-3.php" method="post">
        ...
        <input type="hidden" name="delete_id" value="<?php echo $row['id']; ?>">
            <div class="form-group">
            <h6 style="float: left;">Are you sure you want to reject this Leave?</h6>
            <input class="form-control" type="text" name="remarks" placeholder="Please Add Remarks" required>
            </div>
        </div>
